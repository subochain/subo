...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = "xauto" && test x$bitcoin_qt_force != xyes; then
    if test x$bitcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; subo-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build subo-qt GUI (default=auto, qt5 tried first)])],
    [
...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  dnl enable qt support
  AC_MSG_CHECKING(whether to build Subo Core GUI)
  BITCOIN_QT_CHECK([
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITH_PKGCONFIG],[
  ...
  if test x$auto_priority_version = x; then
    ...
    dnl Append Qt-bundled libpng after the Qt libraries so that the png_*
    dnl references from libQt5Gui.a resolve when Qt is linked statically.
...
if test x$bitcoin_qt_got_major_vers = x5; then
    AC_CHECK_LIB([${QT_LIB_PREFIX}Widgets],[main],,BITCOIN_QT_FAIL(lib$QT_LIB_PREFIXWidgets not found))
  fi
  dnl libqtpng is Qt-bundled libpng. It must come after the Qt libraries so
  dnl that the png_* references pulled in by libQt5Gui.a can be resolved when
  dnl Qt is linked statically.
  QT_LIBS="$LIBS -lqtpng"
  LIBS="$TEMP_LIBS"

...
*Setup instructions for a Gitian build of Subo Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Subo
...
- [Building Subo Core](#building-subo-core)
...
Clone the git repositories for Subo Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/subochain/subo
```
...
in the Subo Core repository under 'Fetch and build inputs' to install sources which require
...
To build Subo Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Subo Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/subo/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/subochain/subo
...
For example:
```bash
URL=https://github.com/crowning-/subo.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit subo=${COMMIT} --url subo=${URL} ../subo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit subo=${COMMIT} --url subo=${URL} ../subo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit subo=${COMMIT} --url subo=${URL} ../subo/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the subo git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../subo/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/subo.git
SIGPATH=/some/root/path/subo-detached-sigs.git

./bin/gbuild --url subo=${BTCPATH},signature=${SIGPATH} ../subo/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/subo-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/subo-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/subo-osx-build.assert
```
...
[subo/gitian.sigs](https://github.com/subochain/gitian.sigs/) repository, or if that's not possible to create a pull
request.

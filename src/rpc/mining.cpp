...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0) {
        //throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Subo Core is not connected!");
    }

    if (IsInitialBlockDownload()) {
        //throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Subo Core is downloading blocks...");
    }
    ...
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT) && !masternodeSync.IsWinnersListSynced() && !mnpayments.GetBlockPayee(chainActive.Height() + 1, payee)){
        //throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Subo Core is downloading masternode winners...");
    }
    ...
    if (sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED) && !masternodeSync.IsSynced() && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1)) {
        //throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Subo Core is syncing with network...");
    }

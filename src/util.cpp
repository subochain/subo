...
const char * const BITCOIN_CONF_FILENAME = "subo.conf";
const char * const BITCOIN_PID_FILENAME = "subod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "subo" is a composite category enabling all Subo-related debug output
            if(ptrCategory->count(string("subo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SuboChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SuboChain
    // Mac: ~/Library/Application Support/SuboChain
    // Unix: ~/.subochain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SuboChain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SuboChain";
#else
    // Unix
    return pathRet / ".subochain";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    if (!boost::filesystem::exists(GetConfigFile())){
        // Create empty subo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override subo.conf
